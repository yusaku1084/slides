...
<div class="highlight">
worktreeだけの安全装置：<strong>同じブランチの二重チェックアウトをブロック</strong>（cloneには無い）
</div>
...
```text
myapp/       ← 人間：diffを見てレビュー & マージ（mainに居座る）
myapp-a/     ← Claude Code ①（branch: feat-login）
myapp-b/     ← Claude Code ②（branch: fix-cart）
myapp-c/     ← Cursor       （branch: refactor-api）
```

- **1worktree = 1ターミナル**：tmuxのペイン/エディタ分割、1つに1エージェント／迷ったら `git worktree list`
- worktreeは**兄弟ディレクトリ**に置く（IDEが他worktreeまでインデックスして重くなるのを防ぐ）
- 机は**常設で使い回す**：作り直さず中でブランチ切替、並べるのはスクリプトでワンコマンド化
- 並列に任せるのは**互いに独立したタスク**（同じファイルを触らない＝コンフリクト回避）

<!--
トーク原稿（約40秒）:
原則は1エージェントに1worktree、1ブランチ。人間はメインに居座って、レビューとマージ判断だけを握ります。ターミナルはworktreeの数だけ分割して、1つに1エージェント。tmuxでもエディタの分割でも構いません。机は兄弟ディレクトリに置いて、常設で使い回す。タスクごとに中でブランチを切り替えます。最初に机を並べるところはスクリプトでワンコマンド化しました。どの机で何が動いてるかは、プロンプトのブランチ名とgit worktree listで分かります。そして一番大事なのが、並列に任せるのは同じファイルを触らない独立したタスクを選ぶこと。ここを外すとマージが地獄になります。
-->
