...
<!--
トーク原稿（約15秒）:
最近、Claude CodeやCursorを複数同時に走らせる環境を初めて作りました。今日はその作り方と、途中でメインのDocker環境を吹き飛ばした失敗談をセットで話します。
-->
...
<!--
トーク原稿（約20秒）:
動機はシンプルで、任せたい独立タスクが同時に複数あるのに、作業ディレクトリが1つだと取り合いになって並列にならない。そこで、エージェントの数だけ作業机を作れるgit worktreeを使います。
-->
...
```text
$ git worktree add ../myapp-task-a -b task-a

myapp/.git ─── 履歴の実体はここ1つだけ（コピーしない）
   ├── myapp/          (branch: main)
   ├── myapp-task-a/   (branch: task-a)  ← 生えた作業ディレクトリ
   └── myapp-task-b/   (branch: task-b)  ← いくつでも生やせる
```
...
<!--
トーク原稿（約40秒）:
「cloneを4個置けば同じでは？」と思うかもしれません。ただcloneは履歴を丸ごとコピーするので重く、fetchや設定も4重になります。一番差が出るのはレビューで、cloneだとpushしてもらわないと見えない作業が、worktreeならブランチがローカル共有なので、メインからその場でdiffできます。同じブランチを2箇所でcheckoutできない制約も、衝突防止の安全装置になります。
-->
...
- **ブランチ名 = ディレクトリ名** → `git worktree list` で誰が何してるか一目瞭然
- 作成はスクリプトで**ワンコマンド化**（だるいと定着しない）

<!--
トーク原稿（約40秒）:
原則は1エージェントに1worktree、1ブランチ。人間はメインに居座って、レビューとマージ判断だけを握ります。worktreeは兄弟ディレクトリに置くこと。リポジトリの中に置くとIDEのインデックスが重くなります。ブランチ名とディレクトリ名を揃えると、git worktree listで誰が何をしてるか一目瞭然。作成はスクリプトでワンコマンド化しました。ちなみにpnpmならinstallも2個目から速いです。
-->
...
<!--
トーク原稿（約55秒）:
ここから事故です。worktree側でmake upしたら、メインのDocker環境が壊れました。コンテナ名は1セットしかないので、worktree発の壊れた構成が、動いていたメインを上書きするんです。壊れ方は三段構え。まず証明書がgit管理外でworktreeでは空、Apacheが起動しない。次にvendorが別ブランチ作業の名残で500エラー。これは7週間起動しっぱなしのopcacheが不整合を隠していて、再起動で初めて露呈しました。最後はDBのbind mountが相対パスで空、Unknown database。原因は3つとも、git管理外は引き継がれない、です。
-->
...
<!--
トーク原稿（約35秒）:
教訓を一般化するとこれです。worktreeに付いてくるのは、gitが知っているファイルだけ。証明書、vendor、.env、DBデータは、無いか古い残骸の状態で始まります。対策は2つ。worktreeを作ったら「何が無いか」を先に疑う。Dockerのような共有インフラは触らず、動作確認はメインに一本化する。これだけで私の事故は全部防げました。
-->
...
<!--
トーク原稿（約25秒）:
まとめです。使い方はこの1行。1エージェント1worktree1ブランチで、兄弟ディレクトリに置く。cloneより軽く、push不要でその場レビューできる。そしてgit管理外のファイルには気をつけて、Dockerでの動作確認はメインで。ありがとうございました。
-->
